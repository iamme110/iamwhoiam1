...
def crop_to_box_v3_pt(box: Box, img: ImagePt, mask_img: MaskPt, target_size: tuple[int, int], max_box_expansion_factor=1.0, border_size=0):
    ...
    l, r = (l - int(torch.floor(torch.tensor(expand_width_lr / 2.0))) - expand_width_l,
            r + int(torch.ceil(torch.tensor(expand_width_lr / 2.0))) + expand_width_r)
    t, b = (t - int(torch.floor(torch.tensor(expand_height_tb / 2.0))) - expand_height_t,
            b + int(torch.ceil(torch.tensor(expand_height_tb / 2.0))) + expand_height_b)
    img = img[t:b + 1, l:r + 1]
