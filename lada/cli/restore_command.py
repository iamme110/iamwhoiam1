...
def add_restore_subparser(subparsers: argparse._SubParsersAction) -> None:
    examples_header_text = _("Examples:")
    example1_text = _("Restore video with default settings:")
    example1_command = _("%(prog)s restore --input input.mp4")
    example2_text = _("Restore all videos found in the specified directory and save them to a different folder:")
    example2_command = _("%(prog)s restore --input path/to/input/dir/ --output /path/to/output/dir/")
    example3_text = _("Use a GPU-accelerated codec for encoding the restored video:")
    example3_command = _("%(prog)s restore --input input.mp4 --codec hevc_nvenc --crf 20")

    restore = subparsers.add_parser(
        "restore",
        help=_("Restore pixelated video(s)"),
    ...
    group_general = restore.add_argument_group(_("General"))
    ...
    group_general.add_argument("--help", action="help", default=argparse.SUPPRESS, help=_("Show this help message and exit"))

    export = restore.add_argument_group(_("Export (Encoder settings)"))
    ...
    restore.set_defaults(func=restore_main, _parser=restore)
    ...
    for input_path, output_path in zip(input_files, output_files):
        ...
        try:
            process_video_file(
                ...
                temp_dir_path=args.temp_directory,
                device=device,
