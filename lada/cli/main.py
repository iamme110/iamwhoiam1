...
def setup_argparser() -> argparse.ArgumentParser:
    ...
    group_general.add_argument('--sbs', action=argparse.BooleanOptionalAction, default=False, help=_("For 3D SBS (VR) videos, enable per-eye YOLO detection."))
...
def process_video_file(input_path: str, output_path: str, temp_dir_path: str, device: torch.device, mosaic_restoration_model, mosaic_detection_model,
                       mosaic_restoration_model_name, preferred_pad_mode, max_clip_length, encoder: str, encoder_options: str, mp4_fast_start, sbs_mode: bool):
    ...
    frame_restorer = FrameRestorer(device, input_path, max_clip_length, mosaic_restoration_model_name,
                 mosaic_detection_model, mosaic_restoration_model, preferred_pad_mode, sbs_mode=sbs_mode)
    success = True
    ...
    for input_path, output_path in zip(input_files, output_files):
        ...
        try:
            process_video_file(input_path=input_path, output_path=output_path, temp_dir_path=args.temporary_directory, device=device, mosaic_restoration_model=mosaic_restoration_model, mosaic_detection_model=mosaic_detection_model,
                               ...
                               encoder=encoder, encoder_options=encoder_options, mp4_fast_start=args.mp4_fast_start, sbs_mode=args.sbs)
